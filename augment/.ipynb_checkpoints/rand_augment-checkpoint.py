...
def shear_x(image, level, replace):
    ...
    # [1  level
    #  0  1].
    image = contrib_image.transform(wrap(image), [1., level, 0., 0., 1., 0., 0., 0.])
    return unwrap(image, replace)
...
def shear_y(image, level, replace):
    ...
    # [1  0
    #  level  1].
    image = contrib_image.transform(wrap(image), [1., 0., 0., level, 1., 0., 0., 0.])
    return unwrap(image, replace)
...
def distort_image_with_autoaugment(image, augmentation_name):
  ...
  """
    ...
    augmentation_hparams = hp(cutout_const=100, translate_const=250)
...
def distort_image_with_randaugment(image, num_layers, magnitude):
  ...
  """
    ...
    augmentation_hparams = hp(cutout_const=40, translate_const=100)
    available_ops = [
        'AutoContrast', 'Equalize', 'Invert', 'Rotate', 'Posterize', 'Solarize', 'Color', 'Contrast', 'Brightness', 'Sharpness', 'ShearX',
